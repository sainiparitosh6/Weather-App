...
function searchWeather() {
    ...
    if (cityInput.trim() !== "") {
        getWeatherDataByCity(cityInput);
    } else {
...
function getWeatherDataByCity(city) {
    fetch(`https://api.openweathermap.org/data/2.5/weather?q=${city}&units=metric&appid=${API_KEY}`)
        .then(res => res.json())
        .then(data => {
            const { coord, name } = data; // Extract city name from the response
            fetch(`https://api.openweathermap.org/data/2.5/onecall?lat=${coord.lat}&lon=${coord.lon}&exclude=hourly,minutely&units=metric&appid=${API_KEY}`)
                .then(res => res.json())
                .then(weatherData => {
                    showWeatherData(weatherData, name); // Pass city name to showWeatherData
                });
        })
        .catch(error => {
...
function showWeatherData (data , cityName){
    ...
    document.title = cityName ? `Weather in ${cityName}` : 'Weather App';
